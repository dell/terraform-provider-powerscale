...
- `target_certificate_id` (String) The ID of the target cluster certificate being used for encryption.
...
- `target_snapshot_alias` (String) The alias of the snapshot taken on the target cluster after the sync completes. A value of @DEFAULT will reset this field to the default creation value.
...
- `target_snapshot_pattern` (String) The name pattern for snapshots taken on the target cluster after the sync completes.  A value of @DEFAULT will reset this field to the default creation value.
