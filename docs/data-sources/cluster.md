...
subcategory: ""
description: |-
  The cluster attributes - config, identity, nodes, internal_networks and ACS. config includes OneFS version and time zone information. identity displays login information. nodes shows the node drives, node hardware, node partition and node status information. internal_network provides information about a cluster internal network. ACS provides information about ACS(Auto Cluster Setup)
---
